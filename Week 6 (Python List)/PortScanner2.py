...
def convert_ip(ip):
	'''//Function Converts the hostname if not already IP//'''
	try:
		IP(ip)
		return(ip)
	except ValueError:
...
def get_banner(s):
	'''//function grabs any port banner and returns the data recieved//'''
	return s.recv(1024)
...
try:
banner = get_banner(sock) print('[+]Port'+str(port)+' isOpen' +':'+
