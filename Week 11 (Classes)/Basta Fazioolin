class Menu:
  ...
  def __repr__(self):
    return self.name + ", available from {start}h to {end}h".format(start=self.start_time, end=self.end_time)
    
  def calculate_bill(self, purchased_items):
    total = 0
    for item in purchased_items:
      total += self.items.get(item)
    return self.name + " Total: " + str(total) + "$"
...
print(brunch.calculate_bill(["tea", "orange juice", "home fries"]))
...
print(arepas_place.available_menus(10))
